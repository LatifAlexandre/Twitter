...
class mainController
{
	...
	{
		$context->tweetsI = tweetTable::getTweets();
		
		$context->tweets = array_slice($context->tweetsI,0,10);
		return context::SUCCESS;
	}
	...
	public static function inscriptionTraitement($request, $context)
	{
   ...
   public static function tweetByDate($request,$context)
    {
    	//var_dump($request);
		if (isset($request['date']))
		{
			$date = $request['date'];
			//on affiche tout les tweets de la BDD
			$context->tweets = tweetTable::getTweetsbyDate($date);
			
			if (count($context->tweets) == 0){
				return context::ERROR;
			}
